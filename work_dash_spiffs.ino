...
void handleReceivedData(String receivedData) {
  ...
  // Append titles for each type of data
  File dataFile = SPIFFS.open("/data.csv", FILE_APPEND);
  if (!dataFile) {
    Serial.println("Failed to open file for appending");
    return;
  }
  dataFile.println("Heartrate,SpO2,Temperature"); // Titles for each column
  dataFile.close();

  // Get the next token and append the actual data
  token = strtok(NULL, ",");
  ...
  // Save data to data.csv file in SPIFFS
  dataFile = SPIFFS.open("/data.csv", FILE_APPEND);
  if (!dataFile) {
    Serial.println("Failed to open file for appending");
    return;
  }
